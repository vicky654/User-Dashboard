...
const Dsidebar = () => {
    ...
    const handleChildEnter = (cIndex: number, rectFromParent: DOMRect) => {
        const subMenuItem = submenuRefs.current[cIndex];
            ...
            return item.children.some((child) => {
                ...
                if (child.children) {
                    return child.children.some((subChild) =>
                        subChild.path && matchPath({ path: subChild.path, end: false }, currentPath)
                    );
                }
                return false;
            });
    ...
    return (
        <div className={semidark ? 'dark' : ''}>
            <div onMouseLeave={handleMouseLeave} className="flex ">
                {/* Sidebar */}
                <nav className="sidebar  fixed top-0 bottom-0 min-h-screen w-[179px] z-50 shadow bg-white dark:bg-black transition-all duration-300">
                    {/* Toggle Button */}
                    <div className="absolute top-1/2 -right-4 transform -translate-y-1/2">
                        <button
                            type="button"
                            className="w-8 h-8 rounded-full  bg-primary shadow-md flex items-center justify-center hover:bg-gray-700 transition"
                                ...
                                <li key={index} className="relative">
                                    <div
                                        ...
                                        className={`cursor-pointer flex flex-col items-center text-center px-3 py-2 rounded-md transition
                                             ${
                                                isParentActive(item)
                                                    ? 'bg-primary/10 text-primary font-semibold bg-primary text-white'
                ...
                {openIndex !== null && SidebarItems[openIndex]?.children?.length ? (
                    <div
                        className="fixed z-[9999] bg-white dark:bg-gray-800 shadow-lg rounded-lg p-3 w-[230px] overflow-auto"
                            ...
                            {SidebarItems[openIndex].children!.map((child, cIndex) => (
                                ...
                                >
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            `flex items-center gap-2 px-3 py-2 rounded  transition ${
                                                isActive ? 'bg-primary/10 text-primary font-semibold' : 'text-gray-700 dark:text-gray-200'
                                            }`
                                        }
                                        onMouseEnter={(e) =>
                                    ...
                                    >
                                        ...
                                        {child.children?.length && (
                                            <i className="fa fa-angle-right ml-auto text-gray-500" />
                                        )}
                                    </NavLink>
                                </li>
                            ))}
                ...
                {openIndex !== null &&
                    ...
                    <div
                        className="fixed z-[9999] bg-white dark:bg-gray-800 shadow-lg rounded-lg p-2 overflow-auto max-h-[250px]"
                        style={{ top: subsubmenuPosition.top, left: subsubmenuPosition.left, minWidth: '200px' }}
                    >
                        <ul className="space-y-1">
